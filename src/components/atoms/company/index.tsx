import React, { useState } from "react";
import Tabs from "../tabs";

const CompanySection = () => {
  const [activeTab, setActiveTab] = useState(0);

  const tabs = [
    {
      label: "Company Overview",
      id: 0,
    },
    {
      label: "Team Introduction",
      id: 1,
    },
  ];

  return (
    <div>
      <div className="p-5 w-[70%] mx-auto">
        <Tabs tabs={tabs} activeTab={activeTab} setActiveTab={setActiveTab} />
      </div>

      <div className="w-[85%] mx-auto mt-10">
        {!activeTab ? (
          <div className="grid grid-cols-2">
            <div className="">
              <img
                src="https://upload.wikimedia.org/wikipedia/commons/thumb/b/b6/Image_created_with_a_mobile_phone.png/1200px-Image_created_with_a_mobile_phone.png"
                alt="Img"
                className="h-[450px] w-[450px] mx-auto"
              />
            </div>
            <div className="p-4 text-justify">
              What is Lorem Ipsum? Lorem Ipsum is simply dummy text of the
              printing and typesetting industry. Lorem Ipsum has been the
              industry's standard dummy text ever since the 1500s, when an
              unknown printer took a galley of type and scrambled it to make a
              type specimen book. It has survived not only five centuries, but
              also the leap into electronic typesetting, remaining essentially
              unchanged. It was popularised in the 1960s with the release of
              Letraset sheets containing Lorem Ipsum passages, and more recently
              with desktop publishing software like Aldus PageMaker including
              versions of Lorem Ipsum. Lorem ipsum dolor, sit amet consectetur
              adipisicing elit. Temporibus molestias magni, repudiandae odit
              laboriosam adipisci animi minus exercitationem accusamus sunt
              veniam illum, delectus ea aliquam qui sequi. Quae magnam nostrum
              magni repellat eligendi delectus dolore rerum sed fugiat facilis
              ipsa, a sequi eveniet nisi dignissimos suscipit unde libero.
              Exercitationem rerum dignissimos perspiciatis consequuntur sunt
              esse! Esse, odit omnis quo quisquam accusantium ea natus vel quis
              magni deserunt at veritatis unde impedit iste et, corrupti
              nesciunt nam error ipsa, facilis vitae harum fugiat nulla. Cumque,
              fuga? Rerum quod ipsum quibusdam praesentium a, molestias maxime
              nam sit delectus tenetur modi in earum?
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-2">
            <div className="">
              <img
                src="data:image/jpeg;base64,"
                alt="Img"
                className="h-[450px] w-[450px] mx-auto"
              />
            </div>
            <div className="p-4 text-justify">
              Why do we use it? It is a long established fact that a reader will
              be distracted by the readable content of a page when looking at
              its layout. The point of using Lorem Ipsum is that it has a
              more-or-less normal distribution of letters, as opposed to using
              'Content here, content here', making it look like readable
              English. Many desktop publishing packages and web page editors now
              use Lorem Ipsum as their default model text, and a search for
              'lorem ipsum' will uncover many web sites still in their infancy.
              Various versions have evolved over the years, sometimes by
              accident, sometimes on purpose (injected humour and the like).
              Lorem ipsum dolor sit, amet consectetur adipisicing elit.
              Mollitia, recusandae hic aperiam eligendi doloribus enim
              voluptatibus iusto blanditiis doloremque culpa ipsa est obcaecati?
              Cum debitis doloremque nam quas nesciunt suscipit magnam corporis
              laboriosam quaerat aspernatur sit nisi quae dolore repudiandae
              numquam, animi qui ducimus. Voluptatibus sit illo tempore officiis
              nisi similique assumenda rem? A odio quia minus. Voluptatem
              architecto facilis laudantium enim repellat tempora. Dignissimos
              possimus ad alias ipsum, reprehenderit saepe est ex accusamus
              minima velit veritatis veniam officia harum ut, consequatur sint?
              Necessitatibus ipsa culpa dolores numquam doloremque, omnis,
              consequatur consectetur mollitia est animi nulla pariatur quam
              itaque harum!
            </div>
          </div>
        )}
      </div>
    </div>
  );
};

export default CompanySection;
